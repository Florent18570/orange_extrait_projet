...
<form name="formulaire" method="POST" action="tampon.php">
    <input type="hidden" name="tampon" value="LIVEDOC">
    <input type="hidden" name="Application" value="LIVEDOC">

    <input type="hidden" name="Creation_par" value="Creation_par">
    <input type="hidden" name="Role" value="Rôle">
    ...
    <?php if ($_POST['information'] != "historique") { ?>
        ...
        $sql20 = "SELECT * FROM `fiche_nav_LIVEDOC` WHERE `CUID`= '" . $_SESSION['Identifiant'] .  "' ";
        // echo $sql20;
        $result20 = $bdd->query($sql20);

        $sql21 = "SELECT * FROM `fiche_nav_etat_habilitation` WHERE `Application`= 'LIVEDOC' and `CUID`= '" . $_SESSION['Identifiant'] .  "' ";
        // echo $sql21;
        ...
        ?>
            ...
            <?php
            $sql11 = "SELECT `id_habilitation` FROM `fiche_nav_etat_habilitation` WHERE `Application`= 'LIVEDOC' AND `CUID`= '" . $_SESSION['Identifiant'] .  "' ";
            // echo $sql11;
